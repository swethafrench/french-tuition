...
export default function SchedulePage() {
  ...
  // Keep latest data in refs so click handlers always see fresh data
  const latestStudents = useRef<Student[]>([])
  const latestOverrides = useRef<BatchOverride[]>([])

  const month = currentDate.getMonth()
  const year = currentDate.getFullYear()
  const monthStr = `${year}-${String(month+1).padStart(2,'0')}`

  const loadData = async (ms?: string) => {
    const m = ms ?? monthStr
    const [{ data: b }, sRes, oRes] = await Promise.all([
      supabase.from('batch_student_counts').select('*'),
      api.get('/api/students-with-batch'),
      api.get('/api/batch-overrides', { params: { month: m } }),
    ])
    const s = sRes.data ?? []
    const o = oRes.data ?? []
    setBatches(b ?? [])
    setStudents(s)
    setOverrides(o)
    latestStudents.current = s
    latestOverrides.current = o
    setLoading(false)
    return { s, o }
  }
  ...
  const batchesForDay = (date: Date) => {
    ...
    const reg = batches.filter(b => b.days.includes(dow))
    const ovIds = latestOverrides.current.filter(o => o.override_date === dateStr).map(o => o.batch_id)
    const ov = batches.filter(b => ovIds.includes(b.id) && !reg.find(r => r.id === b.id))
    return [...reg, ...ov]
  }

  const buildBatchStudents = (date: Date, batch: Batch, s: Student[], o: BatchOverride[]) => {
    const dateStr = date.toISOString().split('T')[0]
    const dow = date.getDay() === 0 ? 6 : date.getDay() - 1
    ...
    return { combined, available: s.filter(st => !combined.find(bs => bs.id === st.id)) }
  }

  const openPopup = (date: Date, batch: Batch) => {
    const s = latestStudents.current
    const o = latestOverrides.current
    const { combined, available } = buildBatchStudents(date, batch, s, o)
    setSelectedDay(date)
  ...
  const handleAddOverride = async () => {
    ...
    try {
      ...
      const { combined, available } = buildBatchStudents(selectedDay, selectedBatch, s, o)
      setBatchStudents(combined)
      setAvailableForOverride(available)
      setShowAddOverride(false)
      setOverrideStudentId('')
  ...
  const handleRemoveOverride = async (studentId: string) => {
    const ov = latestOverrides.current.find(o => o.student_id === studentId && o.override_date === overrideDate)
                  ...
                  <div key={di} className={`min-h-24 border-r border-gray-100 last:border-0 p-1.5 ${isToday?'bg-blue-50/50':'hover:bg-gray-50'}`}>
                    ...
                    <div className="flex flex-col gap-0.5">
                      {db.map(b=>{const c=color(b.id);return(
                        <button key={b.id} onClick={()=>openPopup(date,b)}
            ...
            {weekDates().map((date,i)=>{
              ...
              return(
                <div key={i} className={`border-r border-gray-100 last:border-0 p-2 ${isToday?'bg-blue-50/30':''}`}>
                  {db.map(b=>{const c=color(b.id);return(
                    <button key={b.id} onClick={()=>openPopup(date,b)} className={`w-full text-left p-2 rounded-lg mb-1.5 ${c.bg} ${c.text} hover:opacity-80`}>
            ...
            {batchesForDay(currentDate).length===0?(
              ...
              <div className="flex flex-col gap-3">
                {batchesForDay(currentDate).map(b=>{const c=color(b.id);return(
                  <button key={b.id} onClick={()=>openPopup(currentDate,b)} className={`w-full text-left p-4 rounded-xl border ${c.bg} ${c.text} ${c.border} hover:opacity-90`}>
